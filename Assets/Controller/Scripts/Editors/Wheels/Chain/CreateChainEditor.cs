...
namespace Controller.Scripts.Editors.Wheels.Chain
{
    ...
    public class CreateChainEditor : CreateWheelEditor
    {
        ...
        private void OnEnable()
        {
            ...
            _showConnectionsProp =
                serializedObject.FindProperty("showConnections");
            ...
            _rightRotationProp =
                serializedObject.FindProperty("rightRotation");
            _rightMesh = serializedObject.FindProperty("rightMesh");
            _rightMaterials = serializedObject.FindProperty("rightMaterials");

            _boxColliderCenter =
                serializedObject.FindProperty("boxColliderCenter");
            _boxColliderSize =
                serializedObject.FindProperty("boxColliderSize");
            _boxColliderMaterial =
                serializedObject.FindProperty("boxColliderMaterial");
            ...
            _chainStraightCount =
                serializedObject.FindProperty("chainStraightCount");
            _chainFrontCurveCount =
                serializedObject.FindProperty("chainFrontCurveCount");
            _chainBackCurveCount =
                serializedObject.FindProperty("chainBackCurveCount");

            _hingeJointBreakForce =
                serializedObject.FindProperty("hingeJointBreakForce");
        ...
        public override void SetUpGUI()
        {
            ...
            GUIUtils.PropFieldGUI(_showConnectionsProp,
                WheelMessages.ShowConnections);

            GUIUtils.HeaderGUI(WheelMessages.LeftChainSettings);
            GUIUtils.PropFieldGUI(_leftRotationProp,
            ...
            GUIUtils.PropFieldGUI(_rightRotationProp,
                WheelMessages.EulerRotation);
            ...
            GUIUtils.PropFieldGUI(_boxColliderCenter,
                WheelMessages.ColliderCenter);
            GUIUtils.PropFieldGUI(_boxColliderSize,
                WheelMessages.ColliderSize);
            GUIUtils.PropFieldGUI(_boxColliderMaterial,
            ...
            GUIUtils.PropFieldGUI(_hingeJointBreakForce,
                WheelMessages.BreakForce);

            GUIUtils.SliderGUI(_chainDistance, 0.1f, 5f,
                WheelMessages.Distance);
            GUIUtils.SliderGUI(_chainSpacing, 0.01f, 1f,
                WheelMessages.Spacing);

            GUIUtils.IntSliderGUI(_chainStraightCount, 1, 100,
                WheelMessages.StraightCount);
            GUIUtils.IntSliderGUI(_chainFrontCurveCount, 1, 100,
                WheelMessages.FrontCurveCount);
            GUIUtils.IntSliderGUI(_chainBackCurveCount, 1, 100,
                WheelMessages.BackCurveCount);
        ...
        {
            _frontRadius = _chainSpacing.floatValue / (2 *
                Mathf.Tan(Mathf.PI /
                          ((_chainFrontCurveCount.intValue - 1) * 2)));
            _backRadius = _chainSpacing.floatValue / (2 *
                Mathf.Tan(
                    Mathf.PI / ((_chainBackCurveCount.intValue - 1) * 2)));

            float heightDifference = Mathf.Abs(_frontRadius - _backRadius);
            float straightDistance = _chainStraightCount.intValue *
                                     _chainSpacing.floatValue;

            _actualStraightDistance = Mathf.Sqrt(
                Mathf.Pow(straightDistance, 2) -
                Mathf.Pow(heightDifference, 2));

            float straightRotationAngle = Mathf.Asin(
                Mathf.Abs(_frontRadius - _backRadius) /
                (_chainStraightCount.intValue *
                 _chainSpacing.floatValue));
            ...
            _leftFrontCurveCenter = new Vector3(0,
                -_chainDistance.floatValue / 2,
                z + _chainSpacing.floatValue / 2);
            _leftBackCurveCenter = new Vector3(0,
                -_chainDistance.floatValue / 2,
                -z - _chainSpacing.floatValue / 2);
            _rightFrontCurveCenter = new Vector3(0,
                _chainDistance.floatValue / 2,
                z + _chainSpacing.floatValue / 2);
            _rightBackCurveCenter = new Vector3(0,
                _chainDistance.floatValue / 2,
                -z - _chainSpacing.floatValue / 2);

            _leftStraightUpperStart = new Vector3(_frontRadius,
                _leftFrontCurveCenter.y, z);
            _leftStraightUpperEnd =
                new Vector3(_backRadius, _leftBackCurveCenter.y, -z);
            _leftStraightLowerStart = new Vector3(-_frontRadius,
                _leftFrontCurveCenter.y, z);
            _leftStraightLowerEnd =
                new Vector3(-_backRadius, _leftBackCurveCenter.y, -z);

            _rightStraightUpperStart = new Vector3(_frontRadius,
                _rightFrontCurveCenter.y, z);
            _rightStraightUpperEnd =
                new Vector3(_backRadius, _rightBackCurveCenter.y, -z);
            _rightStraightLowerStart = new Vector3(-_frontRadius,
                _rightFrontCurveCenter.y, z);
            _rightStraightLowerEnd = new Vector3(-_backRadius,
                _rightBackCurveCenter.y, -z);
        }

        private void CreateLeftChainLinks()
        {
            // Front Left Curve
            CreateCurve(false, true, true, _leftChainLinks,
                _leftFrontCurveCenter, Vector3.left, 0f, 180f);

            // Left Upper Straight
            CreateStraight(false, true, _leftStraightUpperStart,
                _leftStraightUpperEnd, _straightUpperRotationAngle,
                _leftChainLinks);

            // Back Left Curve
            CreateCurve(false, false, true, _leftChainLinks,
                _leftBackCurveCenter, Vector3.right, -180f, 0f);

            // Left Lower Straight
            CreateStraight(false, false, _leftStraightLowerEnd,
                _leftStraightLowerStart, _straightLowerRotationAngle,
                _leftChainLinks);
        }
        ...
        {
            // Front Right Curve
            CreateCurve(true, true, true, _rightChainLinks,
                _rightFrontCurveCenter, Vector3.left, 0f, 180f);

            // Right Upper Straight
            CreateStraight(true, true, _rightStraightUpperStart,
                _rightStraightUpperEnd, _straightUpperRotationAngle,
                _rightChainLinks);

            // Back Right Curve
            CreateCurve(true, false, true, _rightChainLinks,
                _rightBackCurveCenter, Vector3.right, -180f, 0f);

            // Right Lower Straight
            CreateStraight(true, false, _rightStraightLowerEnd,
                _rightStraightLowerStart, _straightLowerRotationAngle,
                _rightChainLinks);
        }
        ...
        )
        {
            ...
            {
                float positionAngle =
                    Mathf.Lerp(0f, Mathf.PI, i / (curveCount - 1));
                if (!isClockwise)
                {
                    positionAngle = Mathf.PI * 2 - positionAngle;
                }

                float z = startingVector.z * Mathf.Cos(positionAngle) -
                          startingVector.x * Mathf.Sin(positionAngle);
                float x = startingVector.z * Mathf.Sin(positionAngle) +
                          startingVector.x * Mathf.Cos(positionAngle);

                Vector3 pivot =
                    new Vector3(center.x + x, center.y, center.z + z);
                float rotationAngle = Mathf.Lerp(startAngle, endAngle,
                    i / (curveCount - 1));
                Vector3 rotation = isLeft
                ...
                GameObject chainLink = CreateChainLink(pivot, linkName + i,
                    isLeft, rotation);
                chainLinks.Add(chainLink);
            }
        }

        private void CreateStraight(bool isLeft, bool isUpper, Vector3 start,
            Vector3 end, float rotationAngle, List<GameObject> chainLinks)
        {
            ...
            {
                Vector3 pivot =
                    start + direction * (i * _chainSpacing.floatValue);
                string linkName = isLeft ? "L " : "R ";
                ...
                Vector3 rotation = isLeft
                    ...
                    : _rightRotationProp.vector3Value;
                rotation.y = rotationAngle;
                GameObject chainLink = CreateChainLink(pivot,
                    linkName + (i - 0.5f), isLeft, rotation);
                chainLinks.Add(chainLink);
            }
        }

        private GameObject CreateChainLink(Vector3 pivot, string objectName,
            bool isLeft, Vector3 rotation)
        {
            rotation.z = -90f;
        ...
        private void AttachBoxCollider(GameObject chainLink)
        {
            ...
            boxCollider.material =
                _boxColliderMaterial.objectReferenceValue as PhysicMaterial;
        }
        ...
        protected override void OnSceneGUI()
        {
            ...
            for (int i = 0; i < transform.childCount; i++)
            {
                ...
                Vector3 rotatedLocalPosition = rotation90DegreesAroundZ *
                                               chainLink.transform
                                                   .localPosition;
