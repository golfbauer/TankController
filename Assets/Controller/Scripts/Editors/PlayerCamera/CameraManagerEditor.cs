...
namespace Controller.Scripts.Editors.PlayerCamera
{
    ...
    public class CameraManagerEditor : TankComponentEditor
    {
        ...
        {
            _cameraSwitchKey =
                serializedObject.FindProperty("cameraSwitchKey");
            _cameraControllers =
                serializedObject.FindProperty("cameraControllers");
        ...
        {
            GUIUtils.PropFieldGUI(_cameraSwitchKey,
                CameraMessages.CameraSwitchKey);
        ...
        private void CameraPositionsGUI()
        {
            for (int i = 0; i < _cameraControllers.arraySize; i++)
            {
                var cameraController =
                    _cameraControllers.GetArrayElementAtIndex(i)
                        .objectReferenceValue as CameraMovementController;
        ...
        private void CameraControllerGUI(
            CameraMovementController movementController, int index)
        {
            GameObject cameraPosition = movementController.gameObject;
            bool foldout =
                EditorPrefs.GetBool("CameraPositionFoldout" + index, false);
            ...
            if (foldout)
            {
                ...
                CameraType newType =
                    (CameraType)EditorGUILayout.EnumPopup(
                        CreateTankMessages.CameraType, currentType);
                if (newType != currentType)
                    ReplaceCameraPosition(cameraPosition, newType, index);
                EditorGUILayout.ObjectField(CreateTankMessages.Transform,
                    cameraPosition.transform, typeof(Transform), true);
        ...
        private void AddCameraPosition()
        {
            if (GUILayout.Button(CameraMessages.AddCameraPosition))
            {
                GameObject cameraPosition = new GameObject("CameraPosition " +
                    (_cameraControllers.arraySize + 1));
                cameraPosition.transform.SetParent(transform);
                cameraPosition.transform.localPosition = Vector3.zero;
                _cameraControllers.InsertArrayElementAtIndex(
                    _cameraControllers.arraySize);
                _cameraControllers
                        .GetArrayElementAtIndex(_cameraControllers.arraySize -
                                                1).objectReferenceValue =
                    GetCameraController(cameraPosition,
        ...
        private void ReplaceCameraPosition(GameObject cameraPosition,
            CameraType newType, int i)
        {
            DestroyImmediate(cameraPosition
                .GetComponent<CameraMovementController>());
            CameraMovementController newController =
                GetCameraController(cameraPosition, newType);
            _cameraControllers.GetArrayElementAtIndex(i)
                .objectReferenceValue = newController;
        }

        private CameraMovementController GetCameraController(
            GameObject cameraPosition, CameraType cameraType)
        {
            CameraMovementController cameraMovementController;
            switch (cameraType)
            {
                case CameraType.ThirdPerson:
                    cameraMovementController = cameraPosition
                        .AddComponent<ThirdPersonCameraMovementController>();
                    break;
                case CameraType.Scoped:
                    cameraMovementController = cameraPosition
                        .AddComponent<ScopedCameraMovementController>();
                    break;
                default:
