...
namespace Controller.Scripts.Editors.Utils
{
    ...
    {
        public static void PropFieldGUI(SerializedProperty property,
            string label = null)
        {
            if (label == null)
        ...
        public static void SliderGUI(SerializedProperty property,
            float leftVal, float rightVal, string label)
        {
            EditorGUILayout.Slider(property, leftVal, rightVal, label);
        }

        public static void IntSliderGUI(SerializedProperty property,
            int leftVal, int rightVal, string label)
        {
        ...
        public static void RemoveButton(SerializedProperty property,
            int index)
        {
    ...
    {
        public static void ShowLabel(GameObject gameObject,
            SerializedProperty showLabel)
        {
            if (!showLabel.boolValue) return;

            var iconContent =
                EditorGUIUtility.IconContent(WheelMessages.ShowLabelType);
            EditorGUIUtility.SetIconForObject(gameObject,
                (Texture2D)iconContent.image);
        }

        public static void DrawArc(Vector3 center, Vector3 normal,
            Vector3 from, float angle, float radius, Color color)
        {
            Handles.color = color;
        ...
        public static void DrawCircleWithDirection(Transform transform,
            Vector3 circleNormal, Vector3 torqueDir, float radius)
        {
            Vector3 worldNormal = transform.TransformDirection(circleNormal);

            if (circleNormal != Vector3.zero)
                DrawCircle(transform, worldNormal, radius,
        ...
        public static void DrawCircle(Transform transform, Vector3 normal,
            float radius, Color color)
        {
            Handles.color = color;
            Quaternion rotation =
                Quaternion.FromToRotation(Vector3.forward, normal);
            Handles.DrawSolidArc(transform.position, normal,
                rotation * Vector3.right, 360, radius);
        }

        public static void DrawArrow(Vector3 startPosition, Vector3 direction,
            float distance, float arrowheadSize)
        {
            Vector3 endPosition =
            ...
            float size)
        {
            ...
            Vector3 right = rotation * Quaternion.Euler(0, 145, 0) *
                            Vector3.forward;
            Vector3 left = rotation * Quaternion.Euler(0, -145, 0) *
                           Vector3.forward;
    ...
    public static class GeneralMessages
    {
        ...
        public const string NotPrefabModeWarning =
            "You must be in prefab mode to use this tool.";

        public const string PrefabModeWarning =
            "You must exit prefab mode to use this tool.";
