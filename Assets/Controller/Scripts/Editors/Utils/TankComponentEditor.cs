...
namespace Controller.Scripts.Editors.Utils
{
    public class TankComponentEditor : Editor
    {
        ...
        public virtual bool DenyAccess()
        {
        ...
        public virtual void UpdateMesh(Transform thisTransform,
            SerializedProperty mesh, SerializedProperty materials)
        {
            MeshRenderer meshRenderer =
                thisTransform.GetComponent<MeshRenderer>();
            if (!meshRenderer)
                meshRenderer =
                    thisTransform.gameObject.AddComponent<MeshRenderer>();

            MeshFilter meshFilter = thisTransform.GetComponent<MeshFilter>();
            if (!meshFilter)
                meshFilter =
                    thisTransform.gameObject.AddComponent<MeshFilter>();

            meshFilter.mesh = mesh.objectReferenceValue as Mesh;
            Material[] meshRendererMaterials =
                new Material[materials.arraySize];
        ...
        )
        {
            ...
            for (int i = 0; i < newMeshColliders.arraySize; i++)
            {
                MeshCollider meshCollider =
                    thisTransform.gameObject.AddComponent<MeshCollider>();
                meshCollider.sharedMesh = newMeshColliders
                    .GetArrayElementAtIndex(i).objectReferenceValue as Mesh;
                meshCollider.convex = true;

                if (colliderMaterial != null)
                    meshCollider.material =
                        colliderMaterial.objectReferenceValue as
                            PhysicMaterial;
            }
        }
        ...
        {
            MeshCollider[] oldMeshCollidersArray =
                thisTransform.GetComponents<MeshCollider>();
        ...
        public virtual BoxCollider UpdateBoxCollider(Transform thisTransform)
        {
            BoxCollider boxCollider =
                thisTransform.GetComponent<BoxCollider>();

            if (!boxCollider)
                boxCollider =
                    thisTransform.gameObject.AddComponent<BoxCollider>();
        ...
        {
            BoxCollider boxCollider =
                UpdateBoxCollider(thisTransform, colliderCenter);
        ...
        {
            BoxCollider boxCollider = UpdateBoxCollider(thisTransform,
                colliderCenter, colliderSize);
            boxCollider.material =
                colliderMaterial.objectReferenceValue as PhysicMaterial;
            ...
            Transform thisTransform)
        {
            SphereCollider sphereCollider =
                thisTransform.GetComponent<SphereCollider>();

            if (!sphereCollider)
                sphereCollider = thisTransform.gameObject
        ...
        public virtual SphereCollider UpdateSphereCollider(
            Transform thisTransform, SerializedProperty radius)
        {
            SphereCollider sphereCollider =
                UpdateSphereCollider(thisTransform);
        ...
        {
            SphereCollider sphereCollider =
                UpdateSphereCollider(thisTransform, radius);
            sphereCollider.material =
                material.objectReferenceValue as PhysicMaterial;
        ...
        )
        {
            ...
            if (rigidbody == null)
                rigidbody =
                    thisTransform.gameObject.AddComponent<Rigidbody>();
        ...
        {
            Rigidbody rigidbody =
                UpdateRigidbody(thisTransform, isKinematic, useGravity);
        ...
        {
            Rigidbody rigidbody = UpdateRigidbody(thisTransform, hullMass,
                isKinematic, useGravity);
