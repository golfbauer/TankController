namespace Controller.Scripts.ImpactCollision.Services
{
    public static class CollisionMessages
    {
        public const string ArmorSettings = "Armor Settings";
        public const string ColorThicknessModifier = "Color Modifier";
        public const string DefaultArmorSection = "Default Armor Section";
        public const string ArmorSections = "Armor Sections";

        public const string Thickness = "Thickness";
        public const string Tolerance = "Tolerance";
        public const string MaterialType = "Material Type";
        public const string ClearArmorSections = "Clear Armor Sections";

        public const string LocalVertices = "Local Vertices";
        public const string AddVertex = "Add Vertex";
        public const string ClearVertices = "Clear Vertices";
        public const string UseColliderVertices = "Use Collider Vertices";

        public const string Highlight = "Highlight";
        public const string UnHighlight = "UnHighlight";
    }
}
