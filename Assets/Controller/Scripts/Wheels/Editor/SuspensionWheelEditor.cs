...
namespace Controller.Scripts.Wheels.SuspensionWheel
{
    ...
    public class SuspensionWheelEditor : WheelEditor
    {
        ...
        private void OnEnable()
        {
            ...
            _rightSuspensionMaterialProp = serializedObject.FindProperty(
                "rightSuspensionMaterials"
            );
            ...
            transform = ((SuspensionWheel)target).gameObject.transform;
        ...
        protected override void OnSceneGUI()
        {
            ...
            {
                var suspension = transform.GetChild(i).gameObject;
                if (
                    !suspension.name.Contains(WheelMessages.LeftSuspension)
                    && !suspension.name.Contains(WheelMessages.RightSuspension)
                )
                    continue;

                var hingeJoint = suspension.GetComponent<HingeJoint>();
                var limits = hingeJoint.limits;

                var center =
                    hingeJoint.connectedAnchor + hingeJoint.connectedBody.transform.position;
                var normal = hingeJoint.axis;

                // This is dependent on the axis direction -> if the axis is not pointing in X dir this will be broken
                var minRadAngle = -limits.min * Mathf.Deg2Rad;
                var from = new Vector3(0, Mathf.Sin(minRadAngle), Mathf.Cos(minRadAngle));
                var angle = limits.max - limits.min;

                var springAngle = hingeJoint.spring.targetPosition * Mathf.Deg2Rad;
                var targetPosition = new Vector3(
                    0,
                    Mathf.Sin(springAngle),
                    Mathf.Cos(springAngle)
                ).normalized;
                targetPosition.y *= -1;

                var radius = 0.1f;

                DrawUtils.DrawArc(center, normal, from, angle, radius, GeneralMessages.LightRed);

                DrawUtils.DrawLine(center, center + targetPosition * radius, WheelMessages.Green);

                DrawUtils.DrawLine(center, center + normal * radius, GeneralMessages.Orange, 3);
            }
        }
        ...
        private void MinAngleSliderGUI(
        ...
        private void MaxAngelSliderGUI(
            SerializedProperty property,
            float leftVal,
            float rightVal,
            string label
        ...
        {
            for (var i = 0; i < _componentCountProp.intValue; i++)
            {
                var leftSuspension = CreateSuspension(true, i);
                var wheel = CreateWheel(true, i, leftSuspension);

                RotateObjectsAroundPivot(
                    leftSuspension.transform,
                    wheel.transform,
                    Vector3.right,
                    _suspensionRotationProp.floatValue
                );

                var rightSuspension = CreateSuspension(false, i);
                wheel = CreateWheel(false, i, rightSuspension);

                RotateObjectsAroundPivot(
                    rightSuspension.transform,
                    wheel.transform,
                    Vector3.right,
                    _suspensionRotationProp.floatValue
                );
        ...
        private GameObject CreateWheel(bool isLeft, int i, GameObject suspension)
        {
            ...
            var wheelDistance = _wheelDistanceProp.floatValue + _componentDistanceProp.floatValue;
            wheelDistance = isLeft ? -wheelDistance : wheelDistance;
            var wheelSpacing =
                i * (_wheelSpacingProp.floatValue + _componentSpacingProp.floatValue)
        ...
        {
            var suspensionName = isLeft
                ? WheelMessages.LeftSuspension
                : WheelMessages.RightSuspension;
            var suspensionDistance =
                _suspensionDistanceProp.floatValue + _componentDistanceProp.floatValue;
            suspensionDistance = isLeft ? -suspensionDistance : suspensionDistance;
            var suspensionSpacing =
                _suspensionSpacingProp.floatValue + _componentSpacingProp.floatValue;
        ...
        {
            var suspensionHingeJoint = suspension.AddComponent<HingeJoint>();
        ...
        public static void RotateObjectsAroundPivot(
            Transform pivot,
        ...
        {
            var pivotRotation = Quaternion.AngleAxis(rotationAngle, rotationAxis);
            var pivotToWheel = objectToRotate.position - pivot.position;

            pivot.rotation *= pivotRotation;
            objectToRotate.rotation *= pivotRotation;

            var newPivotToWheel = pivotRotation * pivotToWheel;
