    ...
    public class CameraUIEditor : TankComponentEditor
    {
        ...
        private void UIGroupGUI(UIGroup group, int index)
        {
            ...
            if (foldout)
            {
                ...
                UIGroupType newType = (UIGroupType)
                    EditorGUILayout.EnumPopup(GeneralMessages.Type, group.type);
        ...
        protected void CreateUIGroupGUI()
        {
            if (GUILayout.Button(GeneralMessages.Add))
            {
                ...
                uiGroups.arraySize++;
                uiGroups.GetArrayElementAtIndex(uiGroups.arraySize - 1).objectReferenceValue =
                    uiGroup;
            }
        }
