        ...
        private void CameraPositionsGUI()
        {
            for (int i = 0; i < _cameraControllers.arraySize; i++)
            {
                var cameraController =
                    _cameraControllers.GetArrayElementAtIndex(i).objectReferenceValue
                    as CameraMovementController;
        ...
        private void CameraControllerGUI(CameraMovementController movementController, int index)
        {
            ...
            if (foldout)
            {
                ...
                CameraType newType = (CameraType)
                    EditorGUILayout.EnumPopup(CameraMessages.CameraType, currentType);
                if (newType != currentType)
                    ReplaceCameraPosition(cameraPosition, newType, index);
                EditorGUILayout.ObjectField(
                    CameraMessages.Transform,
        ...
        private void AddCameraPosition()
        {
            if (GUILayout.Button(CameraMessages.AddCameraPosition))
            {
                GameObject cameraPosition = new GameObject(
                    "CameraPosition " + (_cameraControllers.arraySize + 1)
                );
                ...
                _cameraControllers
                    .GetArrayElementAtIndex(_cameraControllers.arraySize - 1)
                    .objectReferenceValue = GetCameraController(
                    cameraPosition,
            ...
            switch (cameraType)
            {
                case (CameraType.ThirdPerson):
                    cameraMovementController =
                        cameraPosition.AddComponent<ThirdPersonCameraMovementController>();
                    break;
                case CameraType.Scoped:
                    cameraMovementController =
                        cameraPosition.AddComponent<ScopedCameraMovementController>();
                    break;
                default:
