...
namespace Controller.Scripts.Managers.ImpactCollision
{
    ...
    {
        [SerializeField] public List<ArmorSection> armorSections = new();
        ...
        public ArmorSection HandleImpact(Vector3 impactPoint,
            Transform transform)
        {
            foreach (ArmorSection armorSection in armorSections)
            {
                if (armorSection.IsImpactPointWithinArmorSection(impactPoint,
                        transform.position))
                {
                    return armorSection;
                }
