...
namespace Controller.Scripts.Managers.Movement
{
    public class MovementManager : MonoBehaviour
    {
        ...
        private void SetUpInputManager()
        {
            ...
            switch (inputType)
            {
                case MovementInputType.Keyboard:
                    _movementInputManager = gameObject
                        .AddComponent<KeyboardInputMovementManager>();
                    break;
        ...
        private void Update()
        {
            ...
            _rightTorque =
                _rightTrackSpeed > 0 ? _rightTorque : -_rightTorque;
        }
        ...
        {
            _leftTrackSpeed = Mathf.Max(0,
                _leftTrackSpeed - breakDecelerationRate * Time.deltaTime);
            _rightTrackSpeed = Mathf.Max(0,
        ...
        private void PivotTurn()
        {
            float playerHorizontalInput =
                _movementInputManager.horizontalInput;
        ...
        public void CalculateVerticalMovement()
        {
            ...
            if (playerVerticalInput > 0)
            {
                _leftTrackSpeed = Mathf.Lerp(_leftTrackSpeed, maxForwardSpeed,
                    accelerationCurve.Evaluate(Time.deltaTime /
                                               accelerationTime));
            }

            if (playerVerticalInput < 0)
            {
                _leftTrackSpeed = Mathf.Lerp(_leftTrackSpeed,
                    -maxReverseSpeed,
                    decelerationCurve.Evaluate(Time.deltaTime /
                                               decelerationTime));
            }

            _leftTrackSpeed = Mathf.Clamp(_leftTrackSpeed, -maxReverseSpeed,
                maxForwardSpeed);
            _rightTrackSpeed = _leftTrackSpeed;
        }
        ...
        public void CalculateHorizontalMovement()
        {
            ...
            {
                _leftDrag = minTurningDrag;
                _rightDrag = Mathf.Lerp(_rightDrag, turningDrag,
                    Time.deltaTime * 100f);
            }
            ...
            if (playerHorizontalInput < 0)
            {
                _leftDrag = Mathf.Lerp(_leftDrag, turningDrag,
                    Time.deltaTime * 100f);
                _rightDrag = minTurningDrag;
            }
