...
namespace Controller.Scripts.Managers.Turret
{
    ...
    {
        [Tooltip(
            "Will draw rays, to show the directions the camera and turret are pointing at")]
        public bool debug;
        ...
        [Tooltip(
            "Layers that should be ignored by the turret when aiming (Please add the tanks layers here!)")]
        ...
        private void DrawRay()
        {
            ...
            Vector3 cameraDirection = cameraAimDirection == Vector3.zero
                ? mainCameraTransform.forward
                : cameraAimDirection;
            Ray cameraRay = new Ray(mainCameraTransform.position,
                cameraDirection);
            if (Physics.Raycast(cameraRay, out var hit, maxCollisionDistance,
                    _raycastLayers))
            {
        ...
        private void RotateHorizontally()
        {
            ...
            _targetAngle = Vector3.SignedAngle(transform.forward,
                _directionToTarget, Vector3.up);

            _targetAngle = Mathf.Clamp(_targetAngle, -maxRightRotationAngle,
                maxLeftRotationAngle);
            ...
            if (Mathf.Abs(_targetAngle) < minMovement)
            {
                ...
                float yRotation =
                    Mathf.Atan2(targetForward.x, targetForward.z) *
                    Mathf.Rad2Deg;
            ...
            {
                _currentRotationSpeed = Mathf.Lerp(_currentRotationSpeed,
                    maxRotationSpeed, Time.deltaTime / accelerationTime);
            }
            else
            {
                _currentRotationSpeed = Mathf.Lerp(_currentRotationSpeed, 0,
            ...
            float angleToRotate = Mathf.Sign(_targetAngle) *
        ...
        private void DebugRotation()
        {
            ...
            Debug.DrawRay(transform.position, _directionToTarget,
                Color.green);
            Debug.DrawRay(mainCamera.transform.position, cameraDir,
                Color.red);
