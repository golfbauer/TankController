...
namespace Controller.Scripts.Ammunition
{
    ...
    public class AmmunitionEditor : TankComponentEditor
    {
        ...
        private void AmmunitionTypeGUI(SerializedProperty ammunitionType, int index)
        {
            ...
            string name =
                ammunitionType.objectReferenceValue == null
                    ? "Empty"
            ...
            if (foldout)
            {
                ...
                SerializedProperty ammunitionProp = ammoTypeSerializedObject.FindProperty(
                    "projectile"
                );
                SerializedProperty ammunitionCountProp = ammoTypeSerializedObject.FindProperty(
                    "ammunitionCount"
                );
        ...
        private void MoveUpAndDown(int index)
        {
            ...
            if (
                GUILayout.Button(GeneralMessages.MoveDown)
        ...
        private void Open()
        {
            if (GUILayout.Button(GeneralMessages.Import))
            {
                string assetPath = EditorUtility.OpenFilePanel(
                    AmmunitionMessages.OpenAmmunitionType,
                    _tmpPath,
                    "asset"
                );
                _tmpPath = assetPath;
                ...
                {
                    var fileContent = (AmmunitionType)
                        AssetDatabase.LoadAssetAtPath(assetPath, typeof(AmmunitionType));
                    if (fileContent != null)
                    {
                        serializedObject.Update();
                        _ammunitionTypes.arraySize++;
                        _ammunitionTypes
                            .GetArrayElementAtIndex(_ammunitionTypes.arraySize - 1)
        ...
        private void Create()
        {
            if (GUILayout.Button(GeneralMessages.Add))
            {
                ...
                string assetPath = EditorUtility.SaveFilePanel(
                    ...
                    _tmpPath,
                    AmmunitionMessages.NewAmmunitionTypeName,
                    "asset"
                );
                _tmpPath = assetPath;
